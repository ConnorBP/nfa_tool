...
fn handle_clear_steam() -> Result<String, String> {
    ...
    let _config_vdf = config_dir.join("config.vdf");
    let _loginusers_vdf = config_dir.join("loginusers.vdf");
...
fn steam_encrypt(token: &str, account_name: &str) -> Result<String, String> {
    ...
    unsafe {
        let success = CryptProtectData(
            ...
            Some(&entropy),
            None,
            None, // Option<*const CRYPTPROTECT_PROMPTSTRUCT>
            0x11,
            &mut data_out,
        );
        if success.is_err() {
...
fn write_autologin_user(account_name: &str) -> Result<(), String> {
    unsafe {
        ...
        if result.is_err() {
            return Err(format!("Failed to open registry key: {:?}", result.is_err()));
        }
        ...
        if set_result.is_err() {
            return Err(format!("Failed to set registry value: {:?}", set_result.is_err()));
        }
